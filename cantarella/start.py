...
async def save(client: Client, message: Message):
    ...
    # FIX: Session ek baar banao loop ke bahar
    acc = None
    ...
    for msgid in range(fromID, toID + 1):
        ...
        # FIX: Cancel check — har file se pehle
        if batch_temp.IS_BATCH.get(message.from_user.id):
            break
        ...
        if is_public_link:
            ...
            # FIX: copy_message ke baad bhi cancel check
            if batch_temp.IS_BATCH.get(message.from_user.id):
                break
        ...
        # FIX: Upload ke baad cancel check
        if batch_temp.IS_BATCH.get(message.from_user.id):
            break

        await asyncio.sleep(2)

    # FIX: Session disconnect karo
    if acc:
        try:
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid):
    ...
    try:
        ...
        # FIX: target_chat use karo (dump chat support)
...
async def button_callbacks(client: Client, callback_query: CallbackQuery):
    ...
    if data == "dev_info":
        ...
        # FIX: PREMIUM_TEXT sirf 2 placeholders leta hai (UPI_ID, QR_CODE)
